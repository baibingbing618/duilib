...
namespace DuiLib {
	...
	CMenuWnd* CMenuWnd::CreateMenu(CMenuElementUI* pOwner, STRINGorID xml, POINT point, CPaintManagerUI* pMainPaintManager, CDuiString type, CStdStringPtrMap* pMenuCheckInfo /*= NULL*/, DWORD dwAlignment /*= eMenuAlignment_Left | eMenuAlignment_Top*/)
	{
		CMenuWnd* pMenu = new CMenuWnd;
		pMenu->Init(pOwner, xml, point, pMainPaintManager, type, pMenuCheckInfo, dwAlignment);
		return pMenu;
	}
	...
	void CMenuWnd::Init(CMenuElementUI* pOwner, STRINGorID xml, POINT point,
		CPaintManagerUI* pMainPaintManager, CDuiString type, CStdStringPtrMap* pMenuCheckInfo/* = NULL*/,
	...
	void CMenuWnd::Notify(TNotifyUI& msg)
	{
		...
		{
			if( msg.sType == _T("click") || msg.sType == DUI_MSGTYPE_ITEMCLICK || msg.sType == _T("valuechanged") )
			{
	...
	LRESULT CMenuWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		if( m_pOwner != NULL) {
            ...
            {
                LPTSTR pstr = NULL;
                WORD id = (WORD)_tcstol(m_xml.m_lpstr, &pstr, 10);
                pRoot = builder.Create((UINT)id, m_type, this, &m_pm);
            }
            else
            {
                pRoot = builder.Create(m_xml, UINT(0), this, &m_pm);
            }
			bShowShadow = m_pm.GetShadow()->IsShowShadow();
			m_pm.GetShadow()->ShowShadow(false);
