...
namespace DuiLib {
	...
	void tagTDrawInfo::Parse(LPCTSTR pStrImage, LPCTSTR pStrModify,CPaintManagerUI *pManager)
	{
		...
		if (pManager != NULL && pManager->GetDPIObj()->GetScale() != 100) 
        {
			if (sResType.IsEmpty())
            {
                CDuiString sTempName = sImageName;
                CDuiString sScale;
                sScale.Format(_T("@%d."), pManager->GetDPIObj()->GetScale());
                sTempName.Replace(_T("."), sScale);

                CDuiString sTempNameX = sImageName;
                CDuiString sScaleX;
                sScaleX.Format(_T("@%.2fx."), pManager->GetDPIObj()->GetScale() / 100.0);
                sTempNameX.Replace(_T("."), sScaleX);

                // if (PathFileExists((pManager->GetResourcePath() + sTempName).GetData()))
                if (pManager->FileInResourceExists(sTempName))
                {
                    sImageName = sTempName;
                }
                else if (pManager->FileInResourceExists(sTempNameX))
                {
                    sImageName = sTempNameX;
                }
                else if (pManager->GetDPIObj()->GetScale() > 100)
                {
                    sTempName = sImageName;
                    sTempName.Replace(_T("."), _T("@2x."));
                    if (PathFileExists((pManager->GetResourcePath() + sTempName).GetData()))
                        sImageName = sTempName;
                }
            }
            else
            {
                ...
                CDuiString sTempName = sImageName;
                LPTSTR pstr = NULL;
                int nImageId = _tcstol(sTempName.GetData(), &pstr, 10);
                UINT nScale = pManager->GetDPIObj()->GetScale();
                if (nScale <= 114)
                {
                    nImageId;
                }
                ...
                sTempName.Format(_T("%d"), nImageId);
                sImageName = sTempName;
            }
    ...
    bool CPaintManagerUI::FileInResourceExists(LPCTSTR pstrFilename)
    {
        CDuiString sFile = GetResourcePath();
        if (GetResourceZip().IsEmpty()) {
            sFile += pstrFilename;
            if (PathFileExists(sFile.GetData()))
            {
                return true;
            }
        }
            ...
            HZIP hz = NULL;
            if (IsCachedResourceZip()) hz = (HZIP)GetResourceZipHandle();
            else {
                CDuiString sFilePwd = CPaintManagerUI::GetResourceZipPwd();
#ifdef UNICODE
                char* pwd = w2a((wchar_t*)sFilePwd.GetData());
	...
	void CPaintManagerUI::ApplyAllAttributeList()
	{
	}
